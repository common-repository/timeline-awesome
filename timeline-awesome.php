 ...
 */
define( 'TIMELINE_AWESOME_VERSION', '1.0.6' );
 ...
 * The shortcode.
 *
 * @param string $atts the id attribute.
 */
function timeline_awesome( $atts ) {
	...
	/* The Loop */
	if ( $query->have_posts() && ! empty( $id ) ) {

		wp_enqueue_style( 'ta-timeline-awesome-fontawesome', plugin_dir_url( __FILE__ ) . 'public/css/fontawesome.min.css', array(), '1.0.4', 'all' );
		wp_enqueue_style( 'ta-timeline-awesome', plugin_dir_url( __FILE__ ) . 'public/css/timeline-awesome-public.css', array(), '1.0.4', 'all' );
		wp_enqueue_style( 'ta-timeline-awesome-responsive', plugin_dir_url( __FILE__ ) . 'public/css/responsive.css', array(), '1.0.4', 'all' );
	...
	<style>
		...
		?>
			...
			.timeline-post-<?php echo esc_attr(get_the_ID()); ?> .timeline-marker:before, .timeline-post-<?php echo esc_attr(get_the_ID()); ?> .timeline .swiper-pagination-bullet::before {
				background: <?php echo esc_html( $timeline_dot_color ); ?>;
			}
			<?php } ?>
			...
			.timeline-post-<?php echo esc_attr(get_the_ID()); ?> .timeline-ver-7-timeline__img.timeline-ver-7-timeline__img--picture {
				background: <?php echo esc_html($timeline_bag_icon_color); ?>;
			}
			<?php } ?>

			<?php if(!empty($timeline_border_icon_color)) { ?>
			.timeline-post-<?php echo esc_attr(get_the_ID()); ?> .timeline-ver-7-timeline__img {
				-webkit-box-shadow: 0 0 0 4px <?php echo esc_html($timeline_border_icon_color); ?>, inset 0 2px 0 rgba(0, 0, 0, 0.08), 0 3px 0 4px rgba(0, 0, 0, 0.05);
				box-shadow: 0 0 0 4px <?php echo esc_html($timeline_border_icon_color); ?>, inset 0 2px 0 rgba(0, 0, 0, 0.08), 0 3px 0 4px rgba(0, 0, 0, 0.05);
			}
			<?php } ?>
